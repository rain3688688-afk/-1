...
const DIM_MAP = {
  1: { id: 1, label: "确定感", color: "#d6d3d1", bg: "linear-gradient(135deg, #1c1917 0%, #292524 100%)", totem: "深扎的树根 / 磐石" },
  2: { id: 2, label: "被需要", color: "#fb923c", bg: "linear-gradient(135deg, #431407 0%, #7c2d12 100%)", totem: "温顺的金毛 / 壁炉之火" },
  3: { id: 3, label: "掌控感", color: "#94a3b8", bg: "linear-gradient(135deg, #0f172a 0%, #334155 100%)", totem: "高速公路 / 船长" },
  4: { id: 4, label: "被偏爱", color: "#fb7185", bg: "linear-gradient(135deg, #4c0519 0%, #881337 100%)", totem: "被驯服的狐狸 / 草莓尖尖" },
  5: { id: 5, label: "精神共鸣", color: "#818cf8", bg: "linear-gradient(135deg, #1e1b4b 0%, #312e81 100%)", totem: "互映的镜子 / 深夜书房" },
  6: { id: 6, label: "自由感", color: "#38bdf8", bg: "linear-gradient(135deg, #0c4a6e 0%, #075985 100%)", totem: "飘浮的云 / 落地窗" },
  7: { id: 7, label: "安全距离", color: "#34d399", bg: "linear-gradient(135deg, #064e3b 0%, #065f46 100%)", totem: "林间小鹿 / 降噪耳机" },
  8: { id: 8, label: "秩序感", color: "#60a5fa", bg: "linear-gradient(135deg, #172554 0%, #1e3a8a 100%)", totem: "囤粮松鼠 / 契约" }
};
...
const RESULTS = {
  ...
  8: {
    ...
    sections: [
      ...
      { t: "祝福", c: "愿你遇见不回避问题、不敷衍沟通的同行者。愿意和你一起把话说透、把日子捋顺。你的清晰与原则与不将就的坚持，从来都是珍贵的礼物。有章法的相处少内耗，这份踏实的温暖，是你最长久的幸福。" }
    ]
  }
};
...
const QUESTIONS = [
  ...
  { id: 13, text: "伴侣无意中提起了一个优秀的异性朋友，言语间带着欣赏。你心里的第一反应是？", opts: [{t: "酸溜溜。 哪怕知道没事，心里也膈应：你为什么要当着我的面夸别人？难道我不够好吗？", v: 4}, {t: "警铃大作。 我会开始对比：那个人是不是比我更适合他？我是不是有被替代的风险？这种危机感让我很不安。", v: 1}, {t: "想搞清楚界限。 我不反感交友，但我需要确认他们现在到底是什么关系？有没有越界？这一切是否在可控范围内？", v: 3}, {t: "无所谓、装傻。 只要别影响我就行。我不太想深究他们的过去，也不想表现得像个嫉妒狂，太难看了。", v: 7}] },
  ...
  { id: 46, text: "如果爱必须伴随一种痛，你宁愿是？", opts: [{t: "生长痛。 像骨骼拉伸一样，虽然酸痛，但说明我们在付出、在羁绊，关系是成长的。", v: 2}, {t: "钝痛。 哪怕关系变得乏味、沉重，也好过“不知明天你还在不在”的剧烈撕裂痛。", v: 1}, {t: "幻痛。 哪怕分开了，你也像我身体的一部分。但我宁愿隔着距离怀念，也不愿靠太近互相伤害。", v: 7}, {t: "剥离痛。 像撕开粘在身上的面具和伪装。那种把最真实的软肋赤裸裸暴露给你的恐惧和疼痛，是我们抵达灵魂深处的代价。", v: 5}] },
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (savedRes) {
      ...
      setMainId(m); setSubId(s); setDisplayId(m); setView('result');
      return;
    }
    const savedProg = localStorage.getItem('psy_progress');
    if (savedProg) {
      const { q, s } = JSON.parse(savedProg);
      setQIndex(q); setScores(s); setView('onboarding');
    }
  }, []);
  ...
  const handleAnswer = async (val) => {
    ...
    if (nextQ === 16 || nextQ === 32) {
      setTransText(nextQ === 16 ? "现实切片已完成\n正在下潜至情绪暗涌..." : "情绪暗涌已穿越\n准备进入灵魂图腾...");
      setView('transition');
      await wait(3500);
  ...
  const finishQuiz = async (finalScores) => {
    ...
    const s = sorted.length > 1 ? parseInt(sorted[1][0]) : (m === 1 ? 2 : 1);
  ...
  return (
    <div className="app-root">
      ...
      {view === 'landing' && (
        <div className="page center fade-in">
          ...
          <div className="input-group">
            <input value={code} onChange={e=>setCode(e.target.value)} placeholder="在此输入兑换码" className="code-input" />
            <button onClick={handleVerify} disabled={loading} className="btn-main">{loading?'验证中...':'解锁测试'}</button>
          </div>
      ...
      {view === 'quiz' && (
        <div className="page quiz-layout fade-in">
          <div className="progress-container">
            <div className="progress-bar" style={{width: `${(qIndex+1)/48*100}%`}}></div>
          </div>
          <div className="quiz-header">
            <span className="part-tag">{qIndex<16?'Part 1 现实切片':qIndex<32?'Part 2 情绪暗涌':'Part 3 灵魂图腾'}</span>
            <span className="q-num">{qIndex+1} / 48</span>
          </div>
          <div className="q-content slide-up" key={qIndex}>
            <h3 className="q-text">{QUESTIONS[qIndex].text}</h3>
            <div className="opts-list">
              {QUESTIONS[qIndex].opts.map((opt, i) => (
                <div key={i} className="opt-card" onClick={() => handleAnswer(opt.v)}>{opt.t}</div>
              ))}
            </div>
          </div>
        </div>
      ...
      {view === 'transition' && (
        <div className="page center fade-in">
          ...
          <p className="trans-text fade-in-slow">{transText}</p>
        </div>
      )}

      {view === 'result' && R && (
        <div className="page result-layout fade-in" style={{background: META.bg}}>
          <div className="switch-container">
            <div className="switch-track">
              <div className={`switch-item ${displayId===mainId?'active':''}`} onClick={()=>setDisplayId(mainId)}>
                主欲望 {DIM_MAP[mainId].label}
              </div>
              <div className={`switch-item ${displayId===subId?'active':''}`} onClick={()=>setDisplayId(subId)}>
                副欲望 {DIM_MAP[subId].label}
              </div>
            </div>
          </div>
          ...
          <div className="details">
            {R.sections.map((sec, idx) => (
              <div key={idx} className={`section-card ${sec.isForPartner ? 'partner-card' : ''}`}>
                <div className="sec-head" onClick={() => toggleSection(idx)}>
                  <span className="sec-title">{sec.t}</span>
                  <span className={`toggle-icon ${expandedSection[idx]?'open':''}`}>+</span>
                </div>
                <div className={`sec-body ${expandedSection[idx]?'expanded':''}`}>
                  <div className="sec-content">{sec.c}</div>
                </div>
              </div>
            ))}
            <div className="footer">
              <button className="btn-outline" onClick={() => {localStorage.clear(); location.reload();}}>重新测试</button>
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
        ...
        .noise-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-image: url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)' opacity='0.05'/%3E%3C/svg%3E"); pointer-events: none; z-index: 50; }
        
        .page { min-height: 100vh; position: relative; z-index: 1; }
        .center { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 32px; text-align: center; }
        ...
        /* 落地页 */
        .brand-pill { font-size: 12px; letter-spacing: 2px; opacity: 0.6; margin-bottom: 24px; border: 1px solid var(--glass-border); padding: 4px 12px; border-radius: 20px; }
        .title { font-family: var(--font); font-size: 40px; font-weight: 300; margin: 0 0 8px; letter-spacing: 4px; text-shadow: 0 0 20px rgba(255,255,255,0.3); }
        .subtitle { font-size: 10px; letter-spacing: 6px; opacity: 0.4; margin-bottom: 40px; }
        .line { width: 1px; height: 60px; background: linear-gradient(to bottom, transparent, var(--text), transparent); margin: 0 auto 40px; opacity: 0.2; }
        .input-group { width: 100%; max-width: 300px; display: flex; flex-direction: column; gap: 16px; margin-top: 32px; }
        .code-input { background: var(--glass); border: 1px solid var(--glass-border); padding: 16px; text-align: center; color: #fff; font-size: 18px; letter-spacing: 2px; outline: none; transition: 0.3s; }
        .code-input:focus { border-color: rgba(255,255,255,0.3); background: rgba(255,255,255,0.05); }
        .btn-main { background: #fff; color: #000; border: none; padding: 16px; font-size: 14px; letter-spacing: 2px; font-weight: 600; cursor: pointer; transition: 0.3s; }
        .btn-main:active { transform: scale(0.98); }
        
        /* 卡片通用 */
        .card { background: var(--glass); backdrop-filter: blur(12px); border: 1px solid var(--glass-border); padding: 32px; border-radius: 2px; box-shadow: 0 8px 32px rgba(0,0,0,0.2); }
        
        /* 答题页 */
        .quiz-layout { padding: 80px 24px 40px; max-width: 600px; margin: 0 auto; display: flex; flex-direction: column; }
        .progress-container { position: fixed; top: 0; left: 0; width: 100%; height: 2px; background: rgba(255,255,255,0.1); }
        .progress-bar { height: 100%; background: #fff; transition: width 0.5s ease; box-shadow: 0 0 10px rgba(255,255,255,0.5); }
        .quiz-header { display: flex; justify-content: space-between; font-size: 10px; letter-spacing: 2px; opacity: 0.5; margin-bottom: 40px; text-transform: uppercase; }
        .q-text { font-family: var(--font); font-size: 22px; line-height: 1.6; font-weight: 300; margin-bottom: 48px; min-height: 100px; }
        .opts-list { display: flex; flex-direction: column; gap: 16px; }
        .opt-card { padding: 20px; border: 1px solid var(--glass-border); cursor: pointer; transition: all 0.3s; font-size: 15px; line-height: 1.5; color: rgba(255,255,255,0.8); background: rgba(0,0,0,0.2); }
        .opt-card:active { background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.3); }
        
        /* 转场动画 */
        .pulse-ring { width: 80px; height: 80px; border: 1px solid rgba(255,255,255,0.2); border-radius: 50%; animation: pulse 3s infinite; margin-bottom: 32px; }
        @keyframes pulse { 0% { transform: scale(0.8); opacity: 0.5; } 50% { transform: scale(1.2); opacity: 0.1; } 100% { transform: scale(0.8); opacity: 0.5; } }
        .trans-text { font-family: var(--font); font-size: 18px; letter-spacing: 2px; line-height: 2; opacity: 0.8; }
        
        /* 结果页 */
        .result-layout { padding: 80px 20px 40px; min-height: 100vh; transition: background 1s ease; }
        .switch-container { position: fixed; top: 20px; left: 0; width: 100%; display: flex; justify-content: center; z-index: 10; }
        .switch-track { background: rgba(0,0,0,0.6); backdrop-filter: blur(10px); padding: 4px; border-radius: 100px; display: flex; border: 1px solid var(--glass-border); }
        .switch-item { padding: 8px 24px; border-radius: 100px; font-size: 12px; cursor: pointer; transition: 0.3s; opacity: 0.6; }
        .switch-item.active { background: #fff; color: #000; opacity: 1; font-weight: 600; }
        
        .poster { text-align: center; margin-top: 40px; border-top: 2px solid rgba(255,255,255,0.1); }
        .poster-top { font-size: 10px; letter-spacing: 4px; opacity: 0.4; margin-bottom: 24px; }
        .res-title { font-family: var(--font); font-size: 42px; margin: 0 0 16px; font-weight: 300; letter-spacing: 4px; }
        .totem-pill { display: inline-block; padding: 6px 16px; border: 1px solid rgba(255,255,255,0.2); border-radius: 50px; font-size: 12px; margin-bottom: 32px; opacity: 0.8; }
        .quote { font-family: var(--font); font-size: 16px; line-height: 1.8; opacity: 0.9; margin-bottom: 32px; font-style: italic; }
        .tags { display: flex; justify-content: center; gap: 8px; margin-bottom: 32px; }
        .tag { font-size: 10px; border: 1px solid rgba(255,255,255,0.1); padding: 4px 8px; opacity: 0.6; }
        .summary { font-size: 14px; line-height: 1.8; text-align: left; opacity: 0.8; background: rgba(0,0,0,0.2); padding: 16px; border-radius: 4px; }
        
        .section-card { border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom: 0; }
        .sec-head { padding: 24px 0; display: flex; justify-content: space-between; align-items: center; cursor: pointer; }
        .sec-title { font-size: 15px; letter-spacing: 1px; font-weight: 500; opacity: 0.9; }
        .toggle-icon { font-size: 18px; opacity: 0.4; transition: 0.3s; }
        .toggle-icon.open { transform: rotate(45deg); }
        
        .sec-body { max-height: 0; overflow: hidden; transition: max-height 0.5s ease; }
        .sec-body.expanded { max-height: 1000px; }
        .sec-content { padding-bottom: 32px; font-size: 14px; line-height: 2; opacity: 0.75; white-space: pre-line; text-align: justify; }
        
        .partner-card { background: rgba(255,255,255,0.05); padding: 0 20px; margin: 20px -20px; border: none; }
        .partner-card .sec-head { border-bottom: 1px solid rgba(255,255,255,0.05); }
        
        .footer { text-align: center; padding: 60px 0; opacity: 0.3; font-size: 10px; letter-spacing: 2px; }
        .btn-outline { background: transparent; border: 1px solid rgba(255,255,255,0.3); color: #fff; padding: 10px 24px; border-radius: 50px; cursor: pointer; margin-bottom: 20px; font-size: 12px; }
      `}</style>
    </div>
  );
}
