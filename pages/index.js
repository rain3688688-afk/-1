...
// import Head from 'next/head'; // [生产环境请取消注释]
// import { createClient } from '@supabase/supabase-js'; // [生产环境请取消注释]
import { Radar, RadarChart, PolarGrid, PolarAngleAxis, PolarRadiusAxis, ResponsiveContainer } from 'recharts';
// import html2canvas from 'html2canvas'; // [生产环境请取消注释]

// --- [预览环境模拟] ---
// 在你的本地项目中，请删除下面这些模拟代码，并取消上面 import 的注释
const html2canvas = async (element, options) => {
  console.log("Mock html2canvas called");
    ...
    select: (columns) => ({
      eq: (column, value) => ({
        single: async () => {
            // 模拟验证兑换码成功
            if (table === 'codes') {
                return { data: { used_count: 0, max_uses: 1, code: value }, error: null };
            }
...
// 生产环境请使用: const supabase = createClient(supabaseUrl, supabaseKey);
...
const RESULTS = {
  "确定感": {
    ...
    radarColor: "#3b82f6",
    base: `在亲密关系中，你核心的情感诉求是 “稳定与可预期”，始终坚守着对长期联结的极致追求。相较于外在条件的光鲜或浪漫形式的轰轰烈烈，你更看重关系中的确定性—— 凡事有交代、件件有着落、事事有回音，这种清晰、可靠的互动模式，是你构建情感安全感的基石。\n\n你的情感底色厚重且带着强烈的契约精神，对感情的投入从不浅尝辄止，而是带着 “认定即终身” 的郑重。只要对方传递的态度足够明确、信号足够清晰，你便会毫无保留地深度投入，愿意为关系的长久发展付出耐心与精力，包容差异、共担责任，将长期承诺作为情感的核心导向，而非一时的情绪冲动。\n\n这份对关系的厚重期待背后，潜藏着你对不确定性的极度敏感与对被抛弃的深层恐惧。你对关系中的距离变化、态度波动有着本能的警觉，任何模糊的信号、延迟的回应，都可能触发你内心的不安。`,
    lightShadow: [
      { type: 'light', label: "稳定的情感支撑力", text: "你是关系中定海神针般的存在，具备极强的抗风险韧性。不会因短期矛盾或外界波动轻易放弃联结，始终以长期视角对待关系，能为对方提供可依赖的情感锚点。" },
      { type: 'light', label: "纯粹的情感忠诚度", text: "你的情感投入具有极强的排他性与专注度，拒绝“骑驴找马”等模糊模式。一旦确立联结，便会毫无保留地全心投入。" },
      { type: 'light', label: "主动的关系修复力", text: "面对关系裂痕，你倾向于解决问题而非逃避冲突。具备主动沟通、磨合调整的意识，始终以“巩固联结、延续稳定”为目标。" },
      { type: 'shadow', label: "对模糊信号的过度敏感", text: "难以耐受关系中的信息空白，当对方出现沉默、回避等不确定行为时，易陷入过度解读与自我归因，将中性信号转化为被否定。" },
      { type: 'shadow', label: "以体谅为名的需求压抑", text: "因恐惧成为对方的“负担”，常以“懂事”为借口，刻意隐藏自身的情感需求与不安。" },
      { type: 'shadow', label: "对未来的焦虑性预判", text: "受怕失去的深层恐惧驱动，习惯对关系走向做负面预设，难以全然接纳当下的美好。" }
    ],
    partner: [
      "你的追问，从不是找茬，是在求一份安心。当你反复问 “去哪了”“爱不爱我” 时，别嫌我烦 —— 那一刻我正被不安包裹。你不用跟我讲道理，只要抱抱我说 “我在，别怕”，就够了。",
    ...
    self: {
      origins: `深度溯源：看见那个时刻在 “扫雷” 的小孩\n\n你对确定感的极致渴求，本质是焦虑型依恋模式的典型呈现。你一定有过这样的童年记忆：那个重要的照顾者（父母或长辈），对你的态度从来没有固定的规律。他们的爱不是稳定的滋养，有时会在你哭闹时立刻抱起安抚，把你宠成掌心的宝贝；有时却对你的需求视而不见，甚至会因为自己的情绪不佳，对你莫名冷淡或责备。\n\n这种反复无常的对待，让小小的你始终搞不懂：“为什么前一刻还好好的，爱就突然消失了？”这份童年里的不安，不是因为你不够好，而是源于照料者不稳定的回应模式。`,
  ...
  "被需要": {
    ...
    radarColor: "#ea580c",
    base: `在亲密关系里，你最核心的需求是靠被对方依赖来确认自己的价值。对别人来说，爱可能是鲜花、情话和浪漫仪式，但对你而言，爱从来都是实打实的联结 —— 是“我能为你做什么”，是 “我的存在能让你少操心”。\n\n你的情感底色里，藏着一种付出才值得被爱的本能认知。你会本能地盯着对方的生活细节：他加班晚了，你会提前煮好热饭；他遇到麻烦了，你会第一时间凑上去帮忙。这些付出不是刻意讨好，只是真心想成为他生活里离不了的人。\n\n可这份对 “被需要” 的执念背后，藏着你最怕的事 —— 怕自己是可有可无的。要是对方凡事都自己扛，你心里会悄悄空落落的：“原来没有我，他也能过得挺好”。`,
    lightShadow: [
      { type: 'light', label: "靠谱的务实支撑力", text: "你是对方生活里最踏实的后盾，从来不会只说漂亮话。比起口头安慰，你更愿意用实打实的行动帮忙，那种 “不管有事没事，我都在” 的态度，能让他真真切切感受到安全感。" },
      { type: 'light', label: "暖心的情感联结力", text: "你总能通过付出和被依赖，把感情粘得很牢。你特别会察言观色，能捕捉到他没说出口的需求。这种被看见、被照顾的感觉，会让他越来越依赖你。" },
      { type: 'light', label: "清醒的互惠平衡感", text: "你不是那种一味付出的傻好人，心里很清楚感情需要你来我往。你愿意为关系兜底，但也盼着对方能看见你的付出，偶尔也回应你的需求。" },
      { type: 'shadow', label: "价值感绑在“被需要”上", text: "你总不自觉把 “他需不需要我” 和 “我重不重要” 绑在一起。要是他暂时能自己处理事情，没怎么依赖你，你就容易胡思乱想，甚至悄悄觉得自己没那么有价值了。" },
      { type: 'shadow', label: "不敢直说“我也想被你需要”", text: "你心里其实很期待他主动找你帮忙，但又怕说出来会给对方压力，更怕被拒绝。所以你宁愿默默付出，等着对方主动来需要自己，慢慢就变成了失落和委屈。" },
      { type: 'shadow', label: "过度承担把自己累垮", text: "因为悄悄担心他累着，也怕他不再需要自己，你会不自觉把好多活都揽过来。久而久之，付出变成了沉重的负担，你陷入 “越付出越累，越累越怕他离开” 的循环里。" }
    ],
    partner: [
    ...
    self: {
      origins: `深度溯源：看见那个 “懂事才被爱” 的小孩\n\n你对被需要的极致渴求，往往与童年里懂事才能被爱的成长体验深度绑定。你可能记得，小时候的你很早就学会了体谅。可能是父母工作忙碌，你悄悄学着做家务；可能是长辈情绪起伏不定，你慢慢练就察言观色的本事。\n\n你渐渐发现，只有当自己能帮上忙、会照顾人时，才能获得更多关注和肯定。一句你真懂事，就成了童年里最温暖的回馈。久而久之，能帮上忙就会被他人需要，被他人需要才能被爱，这样的认知慢慢沉淀在你的潜意识里。`,
      reshape: `重塑：把价值从 “付出” 里解放出来\n\n1. 先学会不付出，也敢安心被爱。\n你习惯了用自己能做什么来证明重要性，却忘了关系的核心是彼此需要，而非自己必须有用。每天睡前花两分钟，回想当天自己没付出却依然被惦记、被关心的小事。\n\n2. 坦然说我也需要被照顾，不用硬撑万能者。\n你总怕麻烦别人，习惯了扮演照顾他人的角色，却忘了核心关系动机里的连结，从来都是双向流动的。你可以试着主动表达自己的需求，不用觉得提要求是自私。\n\n3. 拓展自我价值的边界，不止于被他人需要。\n一个人的价值感越多元，就越不会在单一关系里过度焦虑。投入工作、深耕爱好，你会发现除了爱情，还有很多人在乎你、需要你。`
    },
  ...
  "掌控感": {
    ...
    lightShadow: [
      { type: 'light', label: "清晰的关系梳理力", text: "你总能一眼看穿矛盾的关键，不会让误会越积越深。不管是彼此的界限划分、相处节奏的调整，你都擅长把模糊的事说清楚，推动双方找到都认可的方式。" },
      { type: 'light', label: "务实的问题突破力", text: "面对关系里的卡点，你从不会拖着或逃避。你行动力强，更愿意主动想办法、做调整，而不是任由问题发酵。这份不推活、敢担当的态度，让关系总带着 “往前推进” 的方向感。" },
      { type: 'light', label: "靠谱的关系托底力", text: "和你在一起，对方会觉得特别踏实。你不会让感情停留在 “凑活过” 的状态，会主动规划未来、协调遇到的矛盾，用理性和行动为这段关系兜底。" },
      { type: 'shadow', label: "对无序的极致焦虑", text: "一旦关系脱离预设的轨道 —— 比如对方做事没计划、总回避深入沟通，你会比常人更容易陷入慌乱，觉得整段关系都在摇晃。" },
      { type: 'shadow', label: "独自掌舵的疲惫感", text: "你习惯了做关系里 “把舵的人”，默默扛起梳理节奏、解决问题、规划方向的责任。久而久之，身心俱疲却很难放手，总担心自己一松劲，这段关系就会乱成一团。" },
      { type: 'shadow', label: "同步期待带来的压力", text: "你对自己要求严格，也会不自觉地期待对方能跟上。当对方跟不上你的步调时，你容易感到失望甚至烦躁，却忘了每个人都有自己的处事节奏。" }
    ],
    partner: [
    ...
    self: {
      origins: `深度溯源：看见那个 “必须懂事扛事” 的小孩\n\n你对掌控感的执念，从来不是天生爱操心，而是小时候在无序的环境里，慢慢被逼出来的一种生存本能。小时候的成长里，或许总有些让人心里没底的时刻：可能环境里少了清晰的规则，遇事没人定调，大家慌慌张张没个准头。\n\n你慢慢发现，想要不被这种混乱裹挟，只能自己站出来：主动把事情捋顺、把规则理清。不是你想当 “小大人”，而是那种看着一切脱离轨道、没人托底的无力感，实在太让人害怕了。`,
  ...
  "被偏爱": {
    ...
    lightShadow: [
      { type: 'light', label: "深情且坚定的专注度", text: "一旦认定一个人，你会全身心投入，给出稳定又纯粹的爱意回馈。你的爱不是泛泛的好，而是带着 “只对你” 的专属感，让对方能清晰感受到被珍视。" },
      { type: 'light', label: "细腻入微的洞察力", text: "你能捕捉到别人忽略的细节 —— 记得他随口提的喜好、察觉他没说出口的情绪。这种 “被真正看见” 的体贴，让关系充满温度。" },
      { type: 'light', label: "用心经营独特联结", text: "你擅长创造属于你们的专属记忆，可能是只有彼此懂的暗号，可能是优先考虑对方的习惯，这些 “别人没有” 的小细节，让关系变得格外深刻动人。" },
      { type: 'shadow', label: "对差别对待的极致敏感", text: "对方对别人多一份耐心、多一句关心、多一次迁就，你心里的天平就会悄悄倾斜。不是小气，是怕自己的 “特别” 被稀释，怕那份独有的偏爱慢慢消失。" },
      { type: 'shadow', label: "陷入过度求证的内耗", text: "你会忍不住反复琢磨他的言行，哪怕是一句随口的话，都要从中寻找 “他到底有没有更在乎我” 的答案，越想越焦虑。" },
      { type: 'shadow', label: "怕落空的防御性退缩", text: "一旦感觉自己不再特别，你会下意识收回深情。不是冷漠，是怕自己毫无保留地付出，最后只换来和别人一样的待遇。" }
    ],
    partner: [
    ...
    self: {
      origins: `深度溯源：看见那个渴望 “被优先选择” 的小孩\n\n你对偏爱的执着，并非贪心或矫情，而是核心情感欲望在早期成长中被反复强化的结果 —— 这份对独一份的渴求，往往源于那些自己的在意没被优先回应的成长时刻。\n\n小时候的你，或许总处在一种关注被分摊的环境里：可能是身边人的注意力被其他事占据，你的需求常常要排在后面。慢慢你发现，只有当自己得到不一样的对待 —— 比如一份独属于自己的关注、一次优先考虑的选择，才能真切感受到 “我是重要的”。那些稀缺的特殊时刻，成了童年里最珍贵的心理养分。`,
  ...
  "精神共鸣": {
    ...
    lightShadow: [
      { type: 'light', label: "思想共振的活力感", text: "你带对方的不只是爱情，更是共同成长的可能 —— 一起拓宽认知边界、探索世界，让关系始终保持向上的生命力。" },
      { type: 'light', label: "深层共情的理解力", text: "你懂得 “倾听” 的真正意义，不只是听表面的话，更能读懂背后的想法与情绪，让对方真切感受到 “被完全懂了” 的安心。" },
      { type: 'light', label: "持续焕新的新鲜感", text: "和你在一起从不会陷入枯燥重复，思维的碰撞、观点的交流，总能为关系注入新灵感，永远有聊不完的话题。" },
      { type: 'shadow', label: "浅层关系的低耐受度", text: "如果一段关系长期只停留在 “吃了没”“在干嘛” 的琐事寒暄，没有深入的思想交流，你会慢慢失去存在感，想要抽离。" },
      { type: 'shadow', label: "将“不理解”等同于“不在意”", text: "对方未必不爱你，只是没能跟上你的思维深度，但这种 “聊不到一块” 的距离，在你心里会被放大成 “你不懂我，也不在乎我”。" },
      { type: 'shadow', label: "精神错位的高敏感度", text: "一旦对方的观点、节奏、对世界的态度不再与你同频，你对这段关系的期待会迅速下降，很难再找回最初的热情。" }
    ],
    partner: [
    ...
    self: {
      origins: `深度溯源：看见那个想得太多却没人懂的小孩\n\n你对精神共鸣的执着，从不是 “太复杂” 的矫情，而是核心关系动机在童年被悄悄塑形的结果 —— 那份 “心里有千言万语，却没人心能懂” 的孤独，早就在心底扎了根。\n\n小时候的你，或许总比同龄人多想一层：对世界有奇奇怪怪的疑问，对情绪有细腻难明的感知。你试着把这些想法说出口，却很少被真正倾听。但心底里，却悄悄埋下了一个执拗的期待：希望有个人能看穿你的 “与众不同”，能和你聊那些别人觉得 “无聊” 却让你兴奋的话题。`,
  ...
  "自由感": {
    ...
    lightShadow: [
      { type: 'light', label: "让人舒展的松弛感", text: "你从不会要求对方 “秒回消息”“事事报备”，也不会干涉他的社交圈、兴趣爱好，让对方觉得和你在一起不用紧绷，能安心做自己。" },
      { type: 'light', label: "清晰坚定的边界感", text: "你懂得尊重自己的空间，也从不越界干涉别人的选择，不会用 “爱” 为名绑架对方的生活，这种分寸感让关系始终保持舒展。" },
      { type: 'light', label: "自我负责的情绪力", text: "你很少把所有情绪都丢给对方消化，遇到压力、困惑时，更习惯自己先梳理、自己承担，不会过度依赖对方的安慰。" },
      { type: 'shadow', label: "对“过度靠近”的本能抗拒", text: "只要对方表现出过度依赖、频繁追问、或是试图掌控你的节奏，哪怕没有恶意，你也会下意识想后退，容易让对方误以为你 “不够爱”。" },
      { type: 'shadow', label: "习惯“独自消化”的封闭性", text: "你太擅长自己处理情绪和问题，常常忘了对方也希望参与你的世界 ——哪怕只是听你吐槽、陪你分担，你也会下意识说 “没事”。" },
      { type: 'shadow', label: "对“束缚”的过度预判", text: "只要稍微感受到一点被期待、被要求的压力，你就会立刻警觉，甚至提前撤退，把 “可能被束缚” 的预判当成事实。" }
    ],
    partner: [
    ...
    self: {
      origins: `深度溯源：看见那个 “怕失去自我” 的小孩\n\n你对自由感的执着，从来不是 “不爱黏人” 的天性，而是早年成长体验刻进心底的本能渴望 —— 那些 “不能按自己的想法来、只能听话” 的压抑时刻，早就在心里埋下了对 “自我空间” 的极度珍视。\n\n小时候的你，总被 “听话” 的期待包裹：可能是想法刚说出口就被否定，想做的事总被大人安排好。慢慢的，你在心里划了一道线：只有守住自己的空间和节奏，才不会被别人的期待吞没。不是你不想靠近，是太怕重蹈覆辙，太怕在关系里再次弄丢那个真实的自己。`,
  ...
  "安全距离": {
    ...
    lightShadow: [
      { type: 'light', label: "慎重长情的定心丸", text: "不轻易开启关系，一旦选择便全心投入、坚定不摇，从不会敷衍应付，是关系里最让人安心的 “定海神针”。" },
      { type: 'light', label: "亲疏有度的平衡感", text: "既不会黏腻到让人窒息，也不会疏远到让人不安，总能精准拿捏 “刚刚好” 的距离，让彼此在亲密中保留独立，在独立中感知联结。" },
      { type: 'light', label: "克制稳健的情绪力", text: "从不会把负面情绪随意甩给对方，也不会在冲动下做出伤害关系的决定，总能用理性稳住节奏，用审慎化解矛盾。" },
      { type: 'shadow', label: "节奏错位的孤独感", text: "你的 “慢试探” 总与他人的 “快节奏” 错位，不是不愿靠近，而是很少有人能读懂你 “需要时间确认” 的谨慎。" },
      { type: 'shadow', label: "被催后的防御性退缩", text: "只要对方急于推进关系、用力催促，哪怕没有恶意，你也会下意识后退自保。可对方往往把这份退缩当成 “没兴趣”，误会层层叠加。" },
      { type: 'shadow', label: "观察期的无限内耗", text: "总想着 “再确认一次”“再等等看”，越观察越不敢迈出关键一步，把关系困在原地。" }
    ],
    partner: [
  ...
  "秩序感": {
    ...
    lightShadow: [
      { type: 'light', label: "化繁为简的秩序力", text: "擅长把混乱的相处捋顺，将模糊的边界、零散的期待转化为清晰共识，为关系避开无效内耗，少走很多弯路。" },
      { type: 'light', label: "主动破局的沟通力", text: "遇事从不会逃避冷战，反而主动牵头沟通，愿意和对方一起拆解问题、寻找答案，不把矛盾留到隔夜。" },
      { type: 'light', label: "长远共建的规划力", text: "自带长远视角，不沉溺当下甜蜜，更会从现实出发和对方共建未来规划，让关系在踏实步履中稳步扎根。" },
      { type: 'shadow', label: "对模糊状态零容忍", text: "只要对方态度暧昧、回避沟通，或是用顺其自然敷衍，心里就会立刻陷入焦虑，难以踏实下来。" },
      { type: 'shadow', label: "急于解决却忽略情绪缓冲", text: "习惯遇事立刻推进解决，却忘了亲密关系需要情绪消化的空间，有时候对方要的是安慰，而非马上被梳理成条理。" },
      { type: 'shadow', label: "重秩序易轻共情流动", text: "能快速看清问题本质给出方案，却容易忽略对方的情绪需求 —— 对方或许只是想吐槽发泄，这份理性反而让人觉得不被懂。" }
    ],
    partner: [
